...
function MusicSetup({ onComplete }) {
  ...
  const [artistGenres, setArtistGenres] = useState(Array(TOTAL_ITEMS).fill(''))
  ...
  const handleSelectArtistSuggestion = (index, suggestion) => {
    ...
    setArtistGenres((prev) => {
      const next = [...prev]
      next[index] = (suggestion.primaryGenreName || '').toUpperCase()
      return next
    })
    setArtistSuggestions([])
  ...
  const handleSubmit = (event) => {
    ...
    const genres = [...new Set(artistGenres.map((genre) => genre.trim()).filter(Boolean))]
    saveMusicProfile({ songs, artists, genres })
